...
class VideoAnalyzer(LoggerMixin):
    ...
    def calculate_video_score(self, video_path: str, 
        ...
        try:
            ...
            score = self.calculate_video_score_from_capture(cap, fps, start_time, end_time, total_frames)
            cap.release()
            return score
    ...
    def calculate_video_score_from_capture(self, cap,
                                         fps: float,
                                         start_time: float,
                                         end_time: float,
                                         total_frames: int = 0) -> float:
        """
        基于已打开的视频流计算片段分数，避免重复打开文件。
        """
        try:
            if fps <= 0:
            ...
            normalized_avg = min(avg_motion * 5, 1.0)
            normalized_variance = min(motion_variance * 20, 1.0)
            normalized_max = min(max_motion * 3, 1.0)
